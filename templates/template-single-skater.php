...
function lr_render_single_skater_content() {
    ...
    $transient_key = 'lr_skater_page_html_v2_' . sanitize_key($username);
    ...
    if (is_wp_error($profile_data)) {
        ...
        $output .= '<img src="' . esc_url($avatar_url) . '" onerror="this.onerror=null;this.src=\'' . esc_url($placeholder_url) . '\';" alt="Avatar for ' . esc_attr($display_name) . '" style="border-radius: 50%; width: 150px; height: 150px; margin-bottom: 10px; background-color: #f0f0f0;">';
        ...
        if (!empty($profile_data->publicBio)) { $output .= '<p>' . nl2br(esc_html($profile_data->publicBio)) . '</p>'; } 
        else { $output .= '<p>This skater has not written a bio yet.</p>'; }
