...
/**
 * Retrieves the API access token, handling caching and authentication.
 *
 * This function first checks for a cached token (transient). If not found, it uses
 * the credentials from the plugin's settings to request a new token from the API.
 * It includes robust error handling for connection failures or bad credentials.
 *
 * CRITICAL: The API returns the token prefixed with "Bearer ". This function strips
 * that prefix before caching the token to prevent a "Bearer Bearer" duplication issue
 * when the token is used in `lr_fetch_api_data`.
 *
 * @param bool $force_refresh If true, deletes the cached token to force a new one.
 * @return string|WP_Error The access token on success, or a WP_Error on failure.
 */
function lr_get_api_access_token($force_refresh = false) {
    if ($force_refresh) {
        delete_transient('lr_api_access_token');
...
/**
 * Fetches data from the API with a self-healing authentication and caching layer.
 *
 * This is the central function for all API GET requests. It implements:
 * 1. Caching: Results are cached for 24 hours using the Transients API.
 *    - A unique key is generated from the endpoint and its parameters.
 *    - The LR_CACHE_VERSION constant is used to easily invalidate all caches.
 *    - Caching is bypassed if the plugin's "Testing Mode" is active.
 * 2. Self-Healing Auth: If an API call returns a 401 "Unauthorized" error, this
 *    function automatically attempts to fetch a new access token and retries the
 *    original request once.
 * 3. Error Handling: Returns a WP_Error object if the API call (or the retry) fails,
 *    providing clear feedback.
 *
 * @param string $access_token The API access token (without "Bearer " prefix).
 * @param string $endpoint The API endpoint to call (e.g., 'spots/v2/inBox').
 * @param array $params An associative array of query parameters.
 * @return object|WP_Error The decoded JSON object on success, or a WP_Error on failure.
 */
function lr_fetch_api_data($access_token, $endpoint, $params) {
    global $lr_debug_messages;
    $transient_key = LR_CACHE_VERSION . '_lr_api_cache_' . md5($endpoint . serialize($params));
 ...
 *
 * In a production environment, the user's IP is often hidden behind one or more
 * proxies (e.g., Cloudflare, a load balancer). This function checks a prioritized
 * list of common `$_SERVER` headers to find the true client IP address. It falls
 * back to `REMOTE_ADDR` which is the direct IP of the incoming request.
 *
