...
*   **Intelligent Link Verification Cascade:**
    *   Implemented a robust, multi-step cascade to verify and correct external links in AI-generated content, significantly improving link quality and reliability.
    *   **Step 1: Intelligent Liveness Check:** The system first performs an advanced liveness check that validates HTTP status codes and scans page titles for "soft 404" errors (e.g., "Page Not Found"). It also includes a lenient SSL check to handle servers with outdated certificate bundles.
    *   **Step 2: Refresh Search:** If a link fails the liveness check, the system uses its URL as a query in a "Refresh Search" to find its new, canonical location.
    *   **Step 3: AI-Powered Broad Search:** If the Refresh Search fails, a "Broad Search" is performed using the link's anchor text. The top 5 results are then passed to a Gemini model for contextual evaluation.
    *   **AI as a "Common Sense" Filter:** The AI evaluator acts as a final quality gate, analyzing the search results to pick the most contextually relevant link and, crucially, rejecting all results if none are a good match. This prevents incorrect links (e.g., stock market news for a skate event) from being used.
    *   **Redirect Resolution:** The system now intelligently handles `vertexaisearch.cloud.google.com` redirect URLs generated by the AI, following them to their final destination before starting the verification process.

### 🛠 Under the Hood

*   **Comprehensive Verification Logging:**
    *   The entire link verification cascade is now logged in detail to `link_verification.csv`.
    *   The log includes a unique `Link ID` for each attempt, the original `Link Text`, and clear status notes for each step (e.g., "Failed liveness check," "AI evaluated and chose the best link," "AI found no relevant links"). This provides full transparency into the decision-making process for debugging and analysis.
